...
@DevTestVirtualServer(registryHost = "localhost", deployServiceToVse = "VSE")
----
Next, add VirtualServices rule as a member of Junit Instance. This rule will handle SV as Code annotations during the live cycle of Junit runner engine. Rules allow very flexible addition or redefinition of the behavior of each test method in a test class
[source,java,indent=0]
----	
	@Rule
	public VirtualServicesRule rules = new VirtualServicesRule();
